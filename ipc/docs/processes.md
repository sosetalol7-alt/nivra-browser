...
- **Consult the Platform and IPC teams** (#ipc) to develop the plan for the
  ...
  process (excepting the [launcher process]). There is good reason for this,
...
Code for the complete demo can be found [here](https://phabricator.services.mozilla.com/D119038).
...
Processes](#connecting-with-other-processes), where the parent can be in either the main or a content process.
...
- Ensure your parent class inherits [public ipc::CrashReporterHelper\<GeckoProcessType_Xxx>](https://searchfox.org/mozilla-central/rev/fc4d4a8d01b0e50d20c238acbb1739ccab317ebc/ipc/glue/UtilityProcessParent.h#23)
...
StaticComponents code generation](https://bugzilla.mozilla.org/show_bug.cgi?id=1740485) tracks improving that.
...
- Create the new sandbox definition file
  ...
  it in [Apple Sandbox Guide](https://reverse.put.as/wp-content/uploads/2011/09/Apple-Sandbox-Guide-v1.0.pdf)
...
covered in [Creating a New Top Level Actor](#creating-a-new-top-level-actor). However, before we get into
...
[Creating a New Top Level Actor](#creating-a-new-top-level-actor) below but, first, let's look at how the main
...
[Debugging Process Startup](#debugging-process-startup) looks at what happens if we omit the
...
channel closure because IPDL will call its
{ref}`ActorDestroy <actor-lifetimes-in-c>` method.
...
the New Process](#destroying-the-new-process):
...
the [Child Process Debugging Tool] that allows you to connect to *all*
...
[.childdbg]: https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/-childdbg--debug-child-processes-
[child process debugging tool]: https://marketplace.visualstudio.com/items?itemName=vsdbgplat.MicrosoftChildProcessDebuggingPowerTool
